...
def init_db():
    ...
    try:
        conn.execute("ALTER TABLE cakes ADD COLUMN description TEXT")
    except sqlite3.OperationalError:
        pass
...
def append():
    ...
    if request.method == "POST":
        ...
        conn.execute("INSERT INTO cakes (name, price, image, stock, category_id, description) VALUES (?, ?, ?, ?, ?, ?)",
                 (name, price, image, stock, category_id, description))
        conn.commit()
        conn.close()
        return redirect("/")
...
def edit(id):
    ...
    if request.method == "POST":
        ...
        conn.execute("UPDATE cakes SET name=?, price=?, image=?, stock=?, category_id=?, description=? WHERE id=?",
                 (name, price, image, stock, category_id, description, id))
        conn.commit()
        conn.close()
        return redirect("/")
